...
.risk-item .r-desc{font-size:11px;color:var(--text-dim);margin-top:3px;line-height:1.5;margin-left:24px}
...
<div class="header">
  <div>
    <h1>⸙ nicetoken.top</h1>
    <div style="font-size:12px;color:var(--text-dim);margin-top:2px">全球情绪 · A股情绪 · 板块趋势 · 机会 · 风险 · 资讯</div>
  </div>
...
<div class="grid-2" style="margin-bottom:14px">
  <div class="card pick-card">
    <div class="card-header"><h2>🎯 建议关注</h2><span class="badge">近期投资机会</span></div>
    <div id="pickList"></div>
  </div>
  <div class="card risk-card">
    <div class="card-header"><h2>⚠️ 风险提示</h2><span class="badge">规避风险 · 保护本金</span></div>
    <div id="riskList"></div>
  </div>
</div>
...
const picks=[
  {tag:'🔥',tagType:'hot',title:'半导体 / AI算力',reason:'全球AI资本开支持续高景气，英伟达业绩超预期。关注：<span class="hl">中芯国际</span>、北方华创、寒武纪'},
  {tag:'⚡',tagType:'warm',title:'电力电网 / 特高压',reason:'夏季用电高峰+新能源并网需求。关注：<span class="hl">国电南瑞</span>、许继电气、特变电工'},
  {tag:'📈',tagType:'warm',title:'机器人 / 具身智能',reason:'Optimus量产预期+国内政策扶持。关注：<span class="hl">汇川技术</span>、绿的谐波、拓普集团'},
  {tag:'👀',tagType:'cold',title:'消费电子 / 果链',reason:'Vision Pro新品+AI手机换机预期。关注：<span class="hl">立讯精密</span>、歌尔股份、韦尔股份'}
];
const risks=[
  {level:'high',icon:'🔴',lbl:'高',title:'半导体板块短期过热',desc:'近3月涨幅超25%，部分个股PE已超80倍。获利盘兑现压力大，建议不追高，等待回调至20日均线再考虑'},
  {level:'mid',icon:'🟡',lbl:'中',title:'美联储降息预期反复',desc:'6月非农数据可能超预期，降息时点或推迟至9月。对成长股估值有压制，注意仓位管理'},
  {level:'mid',icon:'🟡',lbl:'中',title:'人民币汇率波动风险',desc:'美元指数短期走强，人民币汇率承压。对进口依赖高的科技企业利润有影响'},
  {level:'low',icon:'🟢',lbl:'低',title:'地缘政治摩擦',desc:'中美科技竞争持续，部分企业可能被列入实体清单。关注标的的供应链风险'}
];
const aiNews=[
...
document.getElementById('distRow').innerHTML=`<div class="dist-item dist-up"><div class="num">2156</div><div class="lbl">涨</div></div><div class="dist-item dist-flat"><div class="num">189</div><div class="lbl">平</div></div><div class="dist-item dist-down"><div class="num">876</div><div class="lbl">跌</div></div>`;
document.getElementById('sentRow').innerHTML=`<div class="sent-item"><div class="v">78</div><div class="l">涨停</div></div><div class="sent-item"><div class="v">3</div><div class="l">跌停</div></div><div class="sent-item"><div class="v">11860</div><div class="l">成交(亿)</div></div><div class="sent-item"><div class="v up">71%</div><div class="l">涨跌比</div></div><div class="sent-item"><div class="v">96%</div><div class="l">封板率</div></div>`;
...
document.getElementById('riskList').innerHTML=risks.map(r=>`<div class="risk-item"><div class="r-title"><span class="r-icon">${r.icon}</span>${r.title}<span class="r-level r-${r.level}">${r.lbl}</span></div><div class="r-desc">${r.desc}</div></div>`).join('');
function rn(el,d,id){document.getElementById(id).textContent=d.length+'条';el.innerHTML=d.map(n=>`<div class="news-item"><div class="title"><span class="date-tag">${n.d}</span><span>${n.t}</span></div><div class="meta"><span>${n.s}</span></div></div>`).join('')}
